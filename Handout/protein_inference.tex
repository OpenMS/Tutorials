...
When analyzing data sets without ground-truth (i.e., we don't know which target identifications are true, which are false,), 
we accept decoy proteins as false positives and target proteins as true positives.
...
As we mentioned above, the classification is not based on the real truth or falsehood. 
Therefore, the so called pseudo-ROC Curve can be generated which plot the number of target 
proteins against the false discovery rate (FDR). 
...
This is similar to the original notion of a ROC curve except that usually the true positives are not known.
...
quality of the score (when the score is interpreted as a probability as Fido does).
