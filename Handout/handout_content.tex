...
\item From the \menu{Work with:} drop down list select the \menu{http://update.knime.org/analytics-platform/2.12}
...
A \textbf{workflow} is a sequence of computational steps applied to a single or multiple input data sets to process and analyse the data.
...
KNIME provides a large number of nodes for a wide range of statistical analysis, machine learning, data processing and visualization. Still, more recent statistical analysis methods, specialized visualizations or cutting edge algorithms may not be covered in KNIME. In order to expand its capabilities over the readily available nodes external scripting languages can be integrated. In this tutorial we primarily use scripts of the powerful statistical compute language R. Note that this part is considered advanced and might be difficult to follow if you are not familiar to R. In this case you might skip this part.

\paragraph{R View (Table)} allow to seamlessly include R scripts into KNIME. We will demonstrate on a minimal example how such a script is integrated.
...
In the \textit{R View (Table)} text editor, enter the following code:
\begin{lstlisting}
...
The table provided as input to the \KNIMENODE{R View (Table) node} is available as R \texttt{data.frame} with name \texttt{knime.in}. Columns (also listed on the left side of the R View window) can be accessed in the usual R way by first specifying the data.frame name and then the column name (e.g. knime.in\$Universe\_0\_0).
\texttt{plot} is the plotting function we use to generate the image. We tell it to use the data in column knime.in\$Universe\_0\_0 as x-coordinate and the column knime.in\$Universe\_0\_1 as y coordinate in the plot. \texttt{main} is simply the plot name and col the column that is used to determine the color (in this case it is the Cluster Membership column).

Now press the \menu{Eval script} and \menu{Show plot} buttons.
\end{task}

\note{Note that we needed to put some extra quotes around Cluster Membership. If we omit those, R would interpret the column name only up to the first space (knime.in\$Cluster) which is not present in the table and leads to an error. Quotes are regularly needed if column names contain spaces or tabs.}
