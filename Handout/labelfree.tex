...
For an easy data analysis of quantitative mass spectrometry-based proteomics data the R Tool MSstats can be used. This tool can be combined via KNIME with a OpenMS preprocessing pipeline. 
In combination with the experimental design in OpenMS all primary information used in MSstats can be obtained from an OpenMS data analysis.
Here we would like to present the a small example how this can be used.  Due to runtime constrictions only the last step and the downstream analysis of the workflow presented in fig. x. will available as example data.  

The Data ..... explanation
Cite Publication
 (https://pubs.acs.org/doi/abs/10.1021/acs.jproteome.6b00881) 

Here experimental design: table 
E.g. explanation of experimental design:
