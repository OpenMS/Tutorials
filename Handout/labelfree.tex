...
For an easy data analysis of quantitative mass spectrometry-based proteomics data the R Tool MSstats can be used. This tool can be combined via KNIME with a OpenMS preprocessing pipeline. In combination with the experimental design in OpenMS all primary information used in MSstats can be obtained from an OpenMS data analysis. Here, we would like to present a example how to utilize these resources.  Due to runtime constrictions in the tutorial session only conversion process and the downstream analysis  will be presented in  further detail. 
