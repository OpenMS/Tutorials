...
\directory{Example\_Data / Metabolomics / databases / MetaboliteSpectralDB.mzML} from the USB stick into the directory of your KNIME installation.
\newline
Navigate to your KNIME folder (On Window \directory{C: / Program Files / KNIME3.3  plugins / de.openms.win32.x86\_64\_2.1.0.201703301405 / payload / share / OpenMS\ CHEMISTRY / }).
Now construct the workflow shown in Figure~\ref{fig:speclib}.
\newline
Use the file \directory{Example\_Data / Metabolomics / datasets / 
\newline
Metabolite\_ID\_SpectraDB\_positive.mzML} as input for your workflow. It contains tandem spectra that are identified by the \KNIMENODE{MetaboliteSpectralMatcher}. The resulting mzTab file is read back into a KNIME table and stored in an Excel table. Make sure that you connect the MzTabReader port corresponding to the Small Molecule Section to the \KNIMENODE{Excel writer (XLS)}).
\end{task}
