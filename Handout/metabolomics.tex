...
For the \KNIMENODE{AccurateMassSearch} results we use the \KNIMENODE{SmallMoleculeMzTabReader} node (\menu{Community Nodes > OpenMS > Conversion > mzTab}), for the result of the \KNIMENODE{TextExporter} we use the \KNIMENODE{ConsensusTextReader} (\menu{Community Nodes > OpenMS > Conversion}).
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{graphics/metabo/metabo_part3}
...
\subsubsection{Bonus task: Visualising data}

Now that you have your data in KNIME you should try to get a feeling for the capabilities of KNIME.

\begin{task}
Check out the \KNIMENODE{Molecule Type Cast} node to render the structural formula contained in the result table.
\end{task}
\begin{task}
...
We finally select only the columns we are interested in with the last \KNIMENODE{Column Filter} and export the data to an Excel file (\KNIMENODE{XLS Writer}).
