...
\label{sec:metaboR}
R Dependencies: This section requires that the R packages \texttt{ggplot2} and \texttt{ggbiplot} are both installed. The suggested way is to first check the availability of the packages in the R binary used in your KNIME by creating a \KNIMENODE{R Source (Workspace)} node with the following short script (in the configure dialog):
\begin{lstlisting}
library(ggplot2)
library(ggfortify)
\end{lstlisting}
Press \menu{Eval script} to execute the script.
You will get an error if a package is not installed yet. In that case, run the following
once (either directly in the KNIME R node or in the R installation that KNIME uses):
\begin{lstlisting}
