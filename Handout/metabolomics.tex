...
Identifying metabolites using only the accurate mass may result in ambiguous results. In practice, additional information (e.g., the retention time) is used to further narrow down potential candidates. Apart from MS1-based features, tandem mass spectra (MS2) of metabolites provide additional information.
...
\begin{task}
Please copy the spectral library: \newline
\directory{Example\_Data \textbackslash Metabolomics \textbackslash databases \textbackslash MetaboliteSpectralDB.mzML} from the USB stick into the directory of your KNIME installation.
\newline
Navigate to your KNIME folder (On Window \directory{C: \textbackslash Program Files \textbackslash KNIME3.3 \newline \textbackslash plugins \textbackslash de.openms.win32.x86\_64\_2.1.0.201703301405 \textbackslash payload \textbackslash share \textbackslash OpenMS \textbackslash CHEMISTRY \textbackslash}).
Now construct the workflow shown in Figure~\ref{fig:speclib}.
Use the file \directory{Example\_Data \textbackslash Metabolomics \textbackslash datasets \newline \textbackslash Metabolite\_ID\_SpectraDB\_positive.mzML} as input for your workflow. It contains tandem spectra that are identified by the \KNIMENODE{MetaboliteSpectralMatcher}. The resulting mzTab file is read back into a KNIME table and stored in an Excel table. Make sure that you connect the MzTabReader port corresponding to the Small Molecule Section to the \KNIMENODE{Excel writer (XLS)}).
\end{task}
...
In metabolomics, matches between tandem spectra and spectral libraries are manually validated. Several commercial and free online resources exist that exist in that task. Some examples are:
