...
//
/noindent Because these libraries tend to be large we don't distribute them with OpenMS. 
...
\directory{Example\_Data / Metabolomics / databases / MetaboliteSpectralDB.mzML}\\ on the USB stick as second input. 
...
\begin{task}
Check in the .xlsx output from the \KNIMENODE{Excel writer (XLS)} if you can find glutathione. In the last column of the "opt_source_idx", which corresponds to the spectrum in the used mzML file. To have look at the fragment spectrum open the mzML file \directory{Example\_Data / Metabolomics / datasets \newline / Metabolite\_ID\_SpectraDB\_positive.mzML} in  TOPPView. To find the corresponding spectra in TOPPView here the MSMS spectrum with the retention time of 67.6 s can be selected in the scan view. Therefore the MS1 spectrum with the retention time of 66.9 s has to be double clicked and the MSMS spectra in this time frame will show up. Select the tandem spectrum of Glutathione, but do not close TOPPView, yet.
\end{task} 
