...
\begin{figure}[htb!]
	\centering
	\includegraphics[width=\textwidth]{graphics/align.png}
...
\noindent Parameter explanation: \\
\textbf{max\_num\_peaks\_considered}: The maximal number of peaks/features to be considered per map. To use all, set this parameter to -1. \\
\textbf{mz\_pair\_max\_distance}: Maximum of m/z deviation of corresponding elements in different maps.  This condition applies to the pairs considered in hashing. \\
\textbf{num\_used\_points}: Maximum number of elements considered in each map (selected by intensity). Use a smaller number to reduce the running time and to disregard weak signals during alignment. \\
\textbf{distance\_RT\_max\_difference}: Features that have a larger RT difference will never be paired. \\
\textbf{distance\_MZ\_max\_difference}: Features that have a larger m/z/ difference will never be paired. \\
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.7\textwidth]{graphics/link.png}
