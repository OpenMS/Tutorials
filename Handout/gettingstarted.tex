...
\newcommand{\WindowsKnimeInstallerName}{Windows / KNIME\ Full\ 3.2.1\ Installer\ (64bit).exe}		
\newcommand{\WindowsOpenMSInstallerName}{Windows / OpenMS-2.1\_Win64\_setup.exe}		
...
\newcommand{\MacKnimeInstallerName}{Mac / knime-full\_3.2.1.macosx.cocoa.x86\_64.dmg}		
\newcommand{\MacOpenMSInstallerName}{Mac / OpenMS-2.1.0\_setup.dmg}		
\newcommand{\KnimeUpdateSite}{http://update.knime.org/analytics-platform/3.2}		
\newcommand{\KnimeTrustedSite}{http://tech.knime.org/update/community-contributions/trusted/3.2}
...
\note{If you chose the KNIME installer that contains all community contributions, or you received the full installer on our USB Stick you can skip this section. This is usually the preferred way but only works for stable releases. For bleeding edge functionality (as provided through the KNIME nightly community contributions or in the case we cover prereleases of OpenMS) you need to perform the actions below.}
...
in the configuration dialog under the OutputTypes tab. After execution, deactivated ports will be marked with a red cross and downstream nodes
will be inactive (inconfigurable).\\
A typical OpenMS workflow in KNIME can be divided in two conceptually different parts:
\begin{itemize}
\item
Nodes for signal and data processing, filtering and data reduction. Here, files are passed between nodes. Execution times of the individual steps are typically longer for these type of nodes as they perform the main computations. 
\item
...
When you are not sure which node to choose next, you have a reasonable suggestion based on other users behavior there.
Connect them to the last node with a double-click.
...
KNIME will export workflows as a knwf file containing all the information on nodes, their connections, and their parameter configuration.
Those knwf files can again be imported by selecting \menu{File > Import KNIME Workflow...}.
...
If you want to check your own workflow by comparing it to the solution or got stuck, simply import the full workflow from the corresponding knwf file.}
...
If you get an error message like "Execute failed: R\_HOME does not contain a folder with name 'bin'." or "Execution failed: R Home is invalid.": please change the R settings in the preferences. To do so open \menu{File > Preferences > KNIME > R} and enter the path to your R installation (the folder that contains the bin directory (e.g., "C: \textbackslash Program Files \textbackslash R \textbackslash R-3.3.1").
