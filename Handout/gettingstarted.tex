...
\newcommand{\WindowsKnimeInstallerName}{Windows / KNIME\ 3.2.1\ Installer\ (64bit).exe}		
...
\newcommand{\MacKnimeInstallerName}{Mac / knime\_3.2.1.app.macosx.cocoa.x86\_64.dmg}		
...
\subsection{Installation of KNIME}

Before we get started we will install the required software. If you take part in a training session you will have likely received an USB stick from us that contains the required data and installer. If we provide laptops with the software you may of course skip the installation process and continue reading the next section.

\subsubsection{Installing KNIME from the USB stick}
...
\subsubsection{Installing KNIME from the internet}
...
\end{itemize}
Choose the installers for the platform you are working on. We suggest to use the full installers of KNIME that includes all free community extensions so that you can skip the installation of other plugins (e.g., the OpenMS plugin s and other dependencies for the example workflows) later.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Building workflows in KNIME}
\label{KNIME_Intro}
...
Other nodes (e.g., used by other software: OpenMS or SeqAn) also use a different port type, so called \textbf{file ports}, representing complete files.
...
\subsection{Installation of OpenMS}
Using OpenMS in combination with KNIME, you can create, edit, open, save, and run workflows
combining OpenMS TOPP tools with the powerful data analysis capabilities of KNIME. Workflows can
...
We now install OpenMS, some prerequisites and required KNIME plugins.

\subsubsection{Installing OpenMS from the USB stick}
Now we install the required OpenMS software. Please execute the:
...
\subsubsection{Installing OpenMS from the internet}
If you are working through this tutorial at home you can get the installers under the following links:
\begin{itemize}
...
\subsection{Analyzing MS data in OpenMS}

\subsubsection{OpenMS data conversion}
...
\subsubsection{OpenMS data visualization using \OPENMSTOOL{TOPPView}}
