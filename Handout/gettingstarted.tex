...
\newcommand{\WindowsDefaultPWizFolder}{C: \textbackslash Program Files \textbackslash OpenMS-2.1.0 \textbackslash share \textbackslash OpenMS \textbackslash THIRDPARTY \textbackslash pwiz-bin}
...
If you have a Windows PC and you want to convert raw data to mzML using ProteoWizard you can either use MSConvertGUI (a graphical user interface) or msconvert (a simple command line tool). Both tools are available in:
\newline
\menu{\WindowsDefaultPWizFolder}.
...
\caption{MSConvertGUI (ProteoWizard), allows converting raw files to mzML. Select the raw files you want to convert by clicking on the browse button. Default parameters can usually be kept as-is. To reduce the initial data size, make sure that the peakPicking filter (converts profile data to centroided data) is listed, enabled (true) and applied to all MS levels (parameter "1-"). Start the conversion process by clicking on the "Start" button.}
\label{fig:toppview}
\end{figure}
...
To convert and pick the file \textit{raw\_data\_file.RAW} you write:
\newline
...
\newline
\note{To display all options you may type \texttt{msconvert --help}. Additional information is available on the ProteoWizard web page.}
