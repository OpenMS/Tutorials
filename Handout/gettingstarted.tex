...
\newcommand{\WindowsKnimeInstallerName}{Windows / KNIME\ 3.6.0\ Installer\ (64bit).exe}	
\newcommand{\WindowsOpenMSInstallerName}{Windows / OpenMS-2.4.0-Win64.exe}	
\newcommand{\WindowsOpenMSPrereqInstallerName}{Windows / OpenMS-2.4-prerequisites-installer.exe}
\newcommand{\WindowsPrerequisitesLink}{http://sourceforge.net/projects/open-ms/files/OpenMS/OpenMS-2.4/OpenMS-2.4-prerequisites-installer.exe/download}
\newcommand{\WindowsDefaultPWizFolder}{C: / Program\ Files / OpenMS-2.4.0 / share / OpenMS / THIRDPARTY / pwiz-bin}
\newcommand{\MacKnimeInstallerName}{Mac / knime\_3.6.0.app.macosx.cocoa.x86\_64.dmg}	
\newcommand{\MacOpenMSInstallerName}{Mac / OpenMS-2.4.0-Darwin.dmg}
\newcommand{\KnimeUpdateSite}{http://update.knime.com/analytics-platform/3.6}
\newcommand{\KnimeTrustedSite}{http://update.knime.com/community-contributions/trusted/3.6}
\newcommand{\KnimeTrunkSite}{https://abibuilder.informatik.uni-tuebingen.de/archive/openms/knime-plugin/updateSite/RC/2.4.0/}
\newcommand{\KnimeUSBUpdateSite}{jar:file:/E:/KNIMEUpdateSite/2.4.0.jar!/}
...
If the app still does not open, you might need to move them from \directory{Applications / OpenMS-2.4.0} to e.g. your Desktop and back.}
...
\note{If you have installed OpenMS on Linux or macOS via your package manager (for instance by installing the \texttt{OpenMS-2.4.0-Linux.deb} package), then you need to set the \texttt{OPENMS\_DATA\_PATH} variable to the directory containing the shared data (normally \texttt{/usr/share/OpenMS}). This must be done prior to running any TOPP tool.}
...
\begin{itemize}
\item Windows: C: \textbackslash Program Files \textbackslash R \textbackslash R-3.5.X (where X is the version you used to install the above libraries)
\item macOS: /Library/Frameworks/R.framework/Versions/3.5/Resources 
\end{itemize}
...
We included a custom KNIME update site to install the OpenMS KNIME plugins from the USB stick.

\begin{itemize}
 % If you base the tutorial on the nightly contributions (not recommended - more of a emergency measure)
  \item \label{it:add_site} In the now open dialog choose \menu{Add...} (in the upper right corner of the dialog) to define a new update site. In the opening dialog enter the following details. \\
  \textit{Name:} \texttt{OpenMS RC 2.4 UpdateSite} \\
  \textit{Location:} 
\end{itemize}  
  \menu{\KnimeUSBUpdateSite}
\begin{itemize}
  \item \label{it:select_site} After pressing \keys{OK} KNIME will show you all the contents of the added Update Site.
  \item \textbf{Note:} From now on, you can use this repository for plugins in the \menu{Work with:} drop-down list.
